...
class MixinStrategies(object):
  ...
  def strategy_rush(self, plan, request, _from, _to):
    ...
    #Если сосед - враг
      ...
      if attackDroids > maxResist * RUSH_ATTACK_RESIST:
        ...
        sendDroids = _from.droids if _to.limit > _from.limit else attackDroids
        request.add(_from.id, _to.id, _from.sendDroids(sendDroids), RUSH)
  ...
  def strategy_support(self, plan, request, _from, _to):
    ...
    elif _to.droids < 10 and _to.maxMyselfNeighboursAttack() > 0:
  ...
  def strategy_redistribution(self, plan, request, _from, _to):
    ...
    #перераспределению
    droidsRedistribution = _from.droids - int(
      _from.limit / ((1 + _from.percent)) ** REDISTRIBUTION_MULTIPLICATOR) + _from.receive_droids
    ...
    if _to.growRating(_to.droids_base) <= 1 and from_growRating_full <= 1:
      ...
      if fromDanger < toDanger:
        ...
        droidsRedistribution2 = _from.droids - int(
          _from.limit / ((1 + _from.percent) ** REDISTRIBUTION_MAX_MULTIPLICATOR)) + _from.receive_droids
